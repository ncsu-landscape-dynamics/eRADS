...
#### Function 1 -- format points and raster ####
# function to reproject spatial point data to North America Albers Equal Area Conic projection
# and reproject raster data into North America Albers Equal Area Conic projection and 1000m resolution
...
raster_format = function(raster_map){
  ...
  raster = projectRaster(raster_map, crs=crs(usall), res=1000)
  return(raster)
}
...
delineate_range = function(presence_pts, coarse_estimation_of_annual_spread_rate= 10000, buffer_size=5000, road_only=F, road_buffer=300){
  ...
  ## detect if there is any outlier ##
  dt = as.data.frame(presence_pts@coords)
  ctd_median = matrix(c(median(dt$coords.x1), median(dt$coords.x2)),nrow=1)
  ctd_median_sp = SpatialPoints(ctd_median, crs(presence_pts))
  dis = gDistance(presence_pts, ctd_median_sp, byid=T)
  dis = as.vector(dis)
  test = chisq.out.test(dis, variance=var(dis), opposite = FALSE)
  outliers = outlier(dis, opposite = FALSE, logical = T)
  
  if (!is.na(test$p.value) & test$p.value<0.5){
    outlier = presence_pts[outliers==T,]
    major_population = presence_pts[outliers==F,]
    
    ctd = gCentroid(major_population)
    
    ## get distance between all major populations and centroid of major infestation populations
    dis=gDistance(ctd,major_population,byid=T)
    ## get maximum distance between centroid and presences
    max_dis = max(dis)
    
    ## calculate minimum surveillance range
    
    # surveillance range for outlier populations
    outlier_surv = gBuffer(outlier, width = buffer_size)
    
    # surveillance range for major infestation populations
    min_surv_size = max_dis + buffer_size
    min_surv = gBuffer(ctd, width = min_surv_size)
    min_surv = gUnion(min_surv, outlier_surv)
    ...
    sug_surv = gUnion(sug_surv, outlier_surv)
...
#' @param coarse_estimation_of_annual_spread_rate coarse estimation of annual spread rate in meter. If based on current infestation status, 
#' @param  buffer_size Size of buffer in meter within which surveillance is desired around a presence
#' @param  road_only Boolean, T (TRUE) or F (FALSE), only survey regions along the road if T
#' @param road_buffer Boolean, T or F, When road_only = T, the surveillance will only be conducted in regions whose distance to road is not further than road_buffer
...
delineate_density = function(confi= 0.75,presence_pts,sensi = 0.15){
...
#' @param host Host map and the generated annual infestation raster will have the same projection, extent, and resolution as the host map
...
p_dtc = function(surv_den,p_den,sensi=0.2){
...
## population density, and detection sensitivity##
surv_den = function(confi=0.75,p_den,sensi=0.2){
...
percent_location = function(density_map, percentage=0.9){
...
percent_population = function(density_map, percentage=0.9){
...
opticonfidence = function(density_map,sensi=0.2,budget=500000,cost_per_survey=70,confidenceMin=0.30,confidenceMax=0.90, number_core){
  require(parallel)
  require(foreach)
  require(doParallel)
...
#' @param confidenceMin & @param confidenceMax Float, accepted minimum and maximum detection confidence for individual survyed pixels
...
infestation_potential = function(density_map, host_map,  dispersal_scale=10, Wcoef=NA, range_buffer=2000, percentage_location=0.9, number_core=5){
...
#' @param range_buffer  A single number; for a given (estimated) invaded pixel, susceptible host within this buffer is considered as potentially can be invaded because of the foci,
...
surv_strategy1 = function(budget=500000, cost_per_survey=70, risk_map, risk_threshold=5, road_only=F, road_buffer=300){
...
#' @param budget Double, budget in dollar
#' @param cost_per_survey Double, estimation of cost per survey site per year, this cost includes all survey cost such as trap cost, labor cost, and facility cost
#' @param road_only Boolean, T or F, if T only survey pixels near to road
...
#' @param risk_map A raster map,value between 0-100, for strategy 1 and strategy 2, estimated invasion risk map
...
#' @param risk_level Boolean T or F, parameter for strategy 2, if T, the risk map will be reclassified into a risk_level map 
...
#' @param density_map A raster map, estimated infestation density map, for strategy 3-7
...
#' @param sensi A value between 0-1, per surveillance sensitivity, which is defined as the probability of detecting an invasion when the surveillance density is 1 and invasion/infestation density is 1   


#### Method 2 -- Risk Based Strategy 2 -- Risk-Weighted Surveillance ####
surv_strategy2 = function(budget=500000, cost_per_survey=70, risk_map, risk_threshold=5 ,road_only=F, road_buffer=300, risk_level=T, number_risk_level=10){
...
surv_strategy3 = function( budget=500000, cost_per_survey=70, density_map, density_threshold=0.2, road_only=F, road_buffer=300){
...
surv_strategy5 = function(use_budget=T, detection_confidence=0.75 , budget, cost_per_survey, density_map, density_threshold, road_only, road_buffer, percentage, location_percentage = T, sensi){
...
#' @param location_percentage Boolean variable, T or F, used for surv_strategy5, if T, the strategy will select the given percentage of estimated infested locations for surveillance; if F, the strategy will select locations which in total have given percentage of estimated infestation populations. Either way, this strategy prioritize locations with higher estimated infestation density/intensity      
...
surv_strategy6 = function(use_budget=T, detection_confidence=0.75 , budget=500000, cost_per_survey=70, density_map, density_threshold=0.2, road_only=F, road_buffer=300, sensi=0.15){
...
surv_strategy7 = function(use_budget=T, detection_confidence=0.75 , budget=500000, cost_per_survey=70, risk_map, density_map, density_threshold=0.2, percentage_location=0.9, road_only=F, road_buffer=300, sensi=0.15, dispersal_scale=10,Wcoef=NA, range_buffer=2000){
...
strategy_evaluation = function(density_map, density_threshold=0.2, surv_den, sensi=0.15){
  density_map[density_map< density_threshold]=0
...
presence_absence_confidence = function(kernel_density_map, presence_pts, absence_pts, surv_density, sensi=0.15){
...
infestation_potential_rank = function(inf_map, host_map,  dispersal_scale=10, Wcoef=NA, range_buffer=2000, number_core=5){
...
ip_treat = function(ip_rank_ply, budget=500000, buffer=100, cost_per_meter_sq=1, inf_map){
...
contain_feasibility = function(inf_map, host_map,estimate_contain_budget =T,  buffer=100, Wcoef=NA, use_lethal_temperature=F, lethal_temperature=-10,
                               lethal_temperature_month = 1, infected, susceptible, total_plants,
